...
| 区域 | 说明 |
| --- | --- |
| 顶部状态栏 | WebSocket 状态、模式、最新时间、日期、历史回放范围。 |
| 左侧控制栏 | 时间窗口、回放点间隔、地图图层、雷达/卫星、测距、锚点、区域边界、粒子谱显示。 |
...
实时模式下，WebSocket 新帧会进入历史数组并刷新地图与图表，左侧“历史长度”默认 10min，需点击旁边确认按钮后生效；“回放点间隔”默认 10s，也使用独立确认按钮。回放模式下，用户通过 slider、播放按钮或地图历史点选取历史帧；历史回放不再按 60min 截断，进入历史模式、播放历史或点击历史航迹点时，详细观测时间轴统一加载当前选中时刻附近 20min。

admin/full 图表权限下，中部地图下方的时序图区域可上下折叠，右侧传感器图表区域可左右折叠。折叠状态只影响当前前端页面，不写入配置；折叠后地图区域通过 CSS grid 自动扩展，并在状态变化后触发地图和图表 resize。

“过冷水汽饱和区识别”标题由面板 header 承担，ECharts 内部 `title.show=false`，避免图内标题挤占绘图区。该图的 legend 与绘图区距离由 `frontend/app.js` 顶部常量控制：`MWR_ZONE_VISUAL_MAP_TOP` 控制 legend 顶部位置，`MWR_ZONE_GRID_TOP` 控制绘图区顶部位置，`MWR_ZONE_GRID_BOTTOM` 控制绘图区底部留白。
